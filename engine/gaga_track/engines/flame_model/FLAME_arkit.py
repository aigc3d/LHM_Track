...
class FLAMEArkitModel(nn.Module):
    """
    ...
    def __init__(self, n_shape, n_exp, scale=1.0, no_lmks=False):
        ...
        # ARKIT 51-dim(except tongout)
        flame_arkit_bs_path = os.path.join(self.flame_path, 'flame_arkit_bs.npy')
        ARKIT_shapedirs = torch.from_numpy(np.load(flame_arkit_bs_path).astype(np.float32)).float().permute(1, 2, 0)[:, :, :51]
        shapedirs = torch.cat([shapedirs[:, :, :n_shape], ARKIT_shapedirs[:, :, :n_exp]], 2)
    ...
    # @torch.no_grad()
    def forward(self, shape_params=None, expression_params=None, pose_params=None, neck_pose_params=None, eye_pose_params=None, verts_sclae=None):
        """
            Input:
        ...
        jaw_params = pose_params[:, 3:] * 0
        full_pose = torch.cat([
                pose_params[:, :3], neck_pose_params, 
                jaw_params, eye_pose_params
            ], dim=1
        )
