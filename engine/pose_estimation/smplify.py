...
class FastFirstFittingLoss(torch.nn.Module):
    def __init__(self, cam_intrinsics, j3d_idx, device, is_smooth=True):
        super().__init__()
...
class SMPLifyLoss(torch.nn.Module):
    def __init__(self, cam_intrinsics, init_pose, j3d_idx, device, is_smooth=True):
...
class TemporalSMPLify:
    ...
    def fit(
        ...
        if cam_intrinsic.dtype == torch.float16:
            ...
            init_transl = inverse_perspective_projection(
                init_loc.unsqueeze(1),
                cam_intrinsic,
                init_dist.unsqueeze(1),
            )[:, 0]
