...
class OptimEngine:
    def __init__(self, model_path, calibration_results, device="cuda"):
        self._device = device
        self.image_size = 512.0
        self.verts_scale = calibration_results["verts_scale"]
        self.focal_length = torch.tensor(
            [calibration_results["focal_length"]], device=device
        )
        # build flame
        self.flame_model = FLAMEModel(
            model_path, n_shape=300, n_exp=100, scale=self.verts_scale
        ).to(device)

    def _build_cameras_kwargs(self, batch_size):
        screen_size = (
            torch.tensor([self.image_size, self.image_size], device=self._device)
            .float()[None]
            .repeat(batch_size, 1)
        )
        cameras_kwargs = {
    ...
    def lightning_optimize(
        ...
        # emica head params
        if optim_neck:
            global_pose = torch.zeros_like(
                batch_base["emica_results"]["globalpose"]
            ).clone()
            head_pose_init = batch_base["emica_results"]["globalpose"].clone()
        else:
            global_pose = batch_base["emica_results"]["globalpose"]
        emica_shapecode = (
            batch_emica["shapecode"].mean(dim=0, keepdim=True).expand(batch_size, -1)
            if share_id
            else batch_emica["shapecode"]
        )
        emica_expcode = batch_emica["expcode"]
        emica_posecode = torch.cat(
            [
                batch_emica["jawpose"].new_zeros(batch_emica["jawpose"].shape),
                batch_emica["jawpose"],
            ],
            dim=1,
        )
        emica_vertices, emica_lmks = self.flame_model(
            ...
            emica_expcode,
            emica_posecode,
            neck_pose_params=head_pose_init if optim_neck else None,
        )
        base_transform_p3d = self.transform_emoca_to_p3d(
            global_pose, emica_vertices, vgg_vertices, self.image_size
        )
        ...
        if optim_neck:
            ...
            identity_rot[:, 2] *= -1
            rotation = (
                matrix_to_rotation_6d(identity_rot)
                .unsqueeze(0)
                .repeat(batch_size, 1)
                .to(ori_rotation.device)
            )
            neck_pose_code = torch.nn.Parameter(
                matrix_to_rotation_6d(
                    euler_angles_to_matrix(head_pose_init.clone(), convention="XYZ")
                ),
                requires_grad=True,
            )
        else:
            base_transform_p3d[:, :3, 3] = (
                base_transform_p3d[:, :3, 3]
                * self.focal_length
                / 13.0
                * self.verts_scale
            ...
            neck_pose_code = (
                matrix_to_rotation_6d(torch.eye(3))
                .unsqueeze(0)
                .repeat(batch_size, 1)
                .to(ori_rotation.device)
            )
        ...
        translation = torch.nn.Parameter(
            base_transform_p3d[:, :, 3], requires_grad=True
        )
        # flame params
        shape_code = torch.nn.Parameter(emica_shapecode.clone(), requires_grad=True)
        exp_code = torch.nn.Parameter(emica_expcode.clone(), requires_grad=True)
        eye_pose_code = torch.nn.Parameter(
            emica_posecode.new_zeros(emica_posecode.shape), requires_grad=True
        )
        optimizer = torch.optim.Adam(
            [
                {"params": [eye_pose_code], "lr": 0.001},
                {"params": [neck_pose_code if optim_neck else rotation], "lr": 0.0005},
                {"params": [translation], "lr": 0.01},
                {"params": [shape_code], "lr": 0.001},
                {"params": [exp_code], "lr": 0.001},
            ]
        )
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=300, gamma=0.1)
        # run
        sapiens_kpts = batch_base["sapiens_kpts"]
        raw_x = batch_base["raw_info"]["image_size"][0][0]
        raw_y = batch_base["raw_info"]["image_size"][1][0]

        if if_crop:
            frame_bbox = batch_base["raw_info"]["frame_bbox"]
            top = frame_bbox[:, 1]
            left = frame_bbox[:, 0]
            height = frame_bbox[:, 3] - frame_bbox[:, 1]
            width = frame_bbox[:, 2] - frame_bbox[:, 0]
            sapiens_kpts[:, :, 0] -= left[:, None]
            sapiens_kpts[:, :, 1] -= top[:, None]
            sapiens_kpts[:, :, 0] *= (512 / width)[:, None]
            sapiens_kpts[:, :, 1] *= (512 / height)[:, None]
        else:
            sapiens_kpts[:, :, 0] *= 512 / raw_x
            sapiens_kpts[:, :, 1] *= 512 / raw_y

        gt_lmks2d70[:, :68] = sapiens_kpts[:, 23:91]
        gt_lmks2d70[:, 68] = sapiens_kpts[:, 2]
        gt_lmks2d70[:, 69] = sapiens_kpts[:, 1]
        lmks_distance = (
            (gt_lmks2d70[:, 17:68] - vgg_lmks2d70[:, 17:68]).norm(dim=-1).mean(dim=-1)
        )
        ...
        for idx in range(steps):
            pred_vertices, pred_lmks = self.flame_model(
                ...
                neck_pose_params=matrix_to_euler_angles(
                    rotation_6d_to_matrix(neck_pose_code), convention="XYZ"
                ),
                pose_params=emica_posecode,
                eye_pose_params=eye_pose_code,
            )
            ...
            # vis_img = torchvision.utils.draw_keypoints(batch_frames[0]/255.0, pred_lmks[0:1], colors="blue", radius=1.5); torchvision.utils.save_image(vis_img, 'test.jpg')
            loss_vertices = mse_loss(project_vertices, vgg_vertices) * 80
            loss_lmks = (
                mse_loss(pred_lmks[:, 17:68], gt_lmks2d70[:, 17:68], mask=lmks_mask)
                * 10
            )
            loss_eyepose = (
                mse_loss(pred_lmks[:, 68:], gt_lmks2d70[:, 68:], mask=lmks_mask) * 25
            )
        ...
        #     {'params': [exp_code], 'lr': 0.001},
        # ])
        # scheduler2 = torch.optim.lr_scheduler.StepLR(optimizer2, step_size=300, gamma=0.1)
        # for idx in range(steps):
        #     pred_vertices, pred_lmks = self.flame_model(
        #         shape_params=shape_code, expression_params=exp_code,
        #         neck_pose_params=matrix_to_euler_angles(rotation_6d_to_matrix(neck_pose_code), convention='XYZ'),
        ...
        #         pred_lmks, R=rotation_6d_to_matrix(rotation), T=translation
        #     )[..., :2]
        #     loss_vertices = 0 #mse_loss(project_vertices, vgg_vertices) * 80
        ...
        for idx, name in enumerate(track_frames):
            optim_results[name] = {
                ...
                "shapecode": shape_code[idx].detach().float().cpu().numpy(),
                "expcode": exp_code[idx].detach().float().cpu().numpy(),
                "posecode": emica_posecode[idx].detach().float().cpu().numpy(),
                "neckcode": matrix_to_euler_angles(
                    rotation_6d_to_matrix(neck_pose_code[idx]), convention="XYZ"
                )
                .detach()
                .float()
                .cpu()
                ...
                "transform_matrix": transform_matrix[idx]
                .detach()
                .float()
                .cpu()
                .numpy(),
            ...
            if if_crop:
                optim_results[name]["frame_bbox"] = (
                    frame_bbox[idx].detach().float().cpu().numpy()
                )
            else:
                optim_results[name]["frame_bbox"] = np.array([])
        if batch_frames is not None:
            with torch.no_grad():
                mesh_render = RenderMesh(
                    512,
                    faces=self.flame_model.get_faces().cpu().numpy(),
                    device=self._device,
                )
                images, alpha_images = mesh_render(
                    pred_vertices[: batch_frames.shape[0]],
                    focal_length=self.focal_length,
                ...
                for idx, frame in enumerate(batch_frames):
                    ...
                    vis_i[alpha_images[idx] > 0.5] += (
                        images[idx, alpha_images[idx] > 0.5] * 0.5
                    )
                    bbox = head_bbox[idx].clone()
                    bbox[[0, 2]] *= vis_i.shape[-1]
                    bbox[[1, 3]] *= vis_i.shape[-2]
                    ...
                    # vis_i = torchvision.utils.draw_keypoints(vis_i, vgg_vertices[idx:idx+1], colors="blue", radius=0.5)
                    vis_i = torchvision.utils.draw_keypoints(
                        vis_i,
                        gt_lmks2d70[idx : idx + 1, 17:],
                        colors="green",
                        radius=1.5,
                    )
                    vis_i = torchvision.utils.draw_keypoints(
                        vis_i, pred_lmks[idx : idx + 1], colors="white", radius=1.5
                    )
                    vis_images.append(vis_i.float().cpu() / 255.0)
                visualization = torchvision.utils.make_grid(vis_images, nrow=10)
        else:
            visualization = None
    ...
    def transform_emoca_to_p3d(emoca_base_rotation, pred_lmks, gt_lmks, image_size):
        ...
        cameras = PerspectiveCameras(
            device=device,
            image_size=torch.tensor([[image_size, image_size]], device=device).repeat(
                batch_size, 1
            ),
        )
        pred_lmks = cameras.transform_points_screen(
            pred_lmks.clone(),
            R=base_transform_p3d[:, :3, :3],
            T=base_transform_p3d[:, :3, 3],
            principal_point=torch.zeros(batch_size, 2),
            focal_length=5000.0 / image_size,
        )[..., :2]
        trans_xy = (
            (pred_lmks.mean(dim=1)[..., :2] - gt_lmks.mean(dim=1)[..., :2])
            * 2
            / image_size
        )
...
def transform_opencv_to_p3d(opencv_transform, verts_scale=1, type="w2c"):
    ...
    if opencv_transform.dim() == 3:
        return torch.stack(
            [transform_opencv_to_p3d(t, verts_scale, type) for t in opencv_transform]
        )
    if type == "c2w":
