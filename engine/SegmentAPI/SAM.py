...
def main():
    ...
    for img in img_list:
        ...
        save_path = os.path.join(opt.output, tu.basename(img) + ".png")
        if not opt.mask:
            save_img = (out.processed_img * 255).astype(np.uint8)
            tu.write_image(save_path, save_img)
            pdb.set_trace()
        else:
