...
def load_bbox_from_local(mask_file):
    ...
    if vis:
        ...
        for kpts, score, visible in zip(
            ...
            for kid, kpt in enumerate(kpts):
                ...
                img = cv2.circle(
                    img, (int(kpt[0]), int(kpt[1])), int(radius), color, -1
                )
...
def main():
    ...
    # data_list = [os.path.join(input_dir, img_name) for img_name in image_names if not os.path.exists(os.path.join("/mnt/data/peihao/HumanVideoProcessing/train_data/deepfashion_processed/sapiens_pose", img_name.replace("jpg", "json")))]
    ...
    for batch_idx, (batch_image_name, batch_orig_imgs, batch_imgs) in tqdm(
        ...
        if use_det:
            ...
            # try to use samurai tracking results instead of mmdet
            bboxes_batch = [
                load_bbox_from_local(image_name.replace("imgs_png", "samurai_seg"))
        ...
        args_list = [
            ...
            for e_i, (i, r, img_name) in enumerate(
                zip(
