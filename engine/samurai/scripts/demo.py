        ...
        if osp.isdir(args.video_path):
            frames = sorted(
                [
                    ...
                    if f.endswith((".jpg", ".jpeg", ".JPG", ".JPEG", ".png", ".PNG"))
                ]
            )
            loaded_frames = [cv2.imread(frame_path) for frame_path in frames]
            frame_ori_idx = [basename(frame_path) for frame_path in frames]
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.float16):
        ...
        for frame_idx, object_ids, masks in predictor.propagate_in_video(state):
            ...
            if args.save_to_video:
                ...
                file_name = frame_ori_idx[frame_idx]
                for obj_id, mask in mask_to_vis.items():

                    img = (img * mask[..., None] + (1 - mask[..., None]) * 255).astype(
                        np.uint8
                    )

                    if args.save_img:
                        mask_png = os.path.join(args.output, file_name + ".png")
                        mask_binary = (mask * 255).astype(np.uint8)
                        cv2.imwrite(mask_png, mask_binary)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_img", action="store_true", help="Save results to a video."
    )
