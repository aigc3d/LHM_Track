...
class SAM2Seg(BaseSeg):
    ...
    def __init__(
        ...
        super().__init__(sam_weight, prior_weight)

        self.device = tu.avaliable_device()
        SAM2_WEIGHT = self.sam_weight
    ...
    def birefnet_predict_bbox(self, img, scale=1.0):
        ...
        # img: RGB-order
        BIREFNET_WEIGHT = self.prior_weight
...
def get_parse():
    ...
    parser.add_argument("--model_path", help="mask bool")
...
def main():
    ...
    opt = get_parse()
    img_list = sorted(tu.is_img(tu.next_files(opt.input)))

    os.makedirs(opt.output, exist_ok=True)
    global SAM2_WEIGHT
    global BIREFNET_WEIGHT
    WORKSPACE = opt.model_path

    SAM2_WEIGHT = join(WORKSPACE, "sam2/sam2.1_hiera_large.pt")
    BIREFNET_WEIGHT = join(WORKSPACE, "BiRefNet-general-epoch_244.pth")
    assert os.path.exists(SAM2_WEIGHT) and os.path.exists(BIREFNET_WEIGHT)

    model = SAM2Seg(wo_supres=True)
    input = opt.input[:-1] if opt.input[-1] == "/" else opt.input
    ...
    for img in img_list[0:1]:
        ...
        save_txt = join(opt.output, tu.basename(input) + ".txt")
